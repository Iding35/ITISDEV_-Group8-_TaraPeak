...
export async function checkWeather(mountainId: number, hikingDate: string): Promise<WeatherCheckResponse> {
  const response = await fetch(`${API_URL}/weather/${mountainId}?date=${hikingDate}`);
